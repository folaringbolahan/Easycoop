        ...
        <div class="col-md-12">
	       ...
	       <div class="panel-body">
			...
			<%--  
			 ...
			 --%>
			   ...
			   <div class="form-group">  
                                <fmt:formatNumber type="number" maxFractionDigits="2" value="${loanRequest.approvedAmount}" var="pcAppAmtVar" />
				  <form:label path="approvedAmount" cssClass="col-sm-4 control-label">Approved Amount:</form:label> 
				  <div class="col-sm-3">
				     <form:input path="approvedAmount" cssClass="error" value="${pcAppAmtVar}" readonly="true"/>
				  </div>  
			   </div> 
	      ...
	      <c:if test="${!empty schedules}">  
				...
				<h4>LOAN REPAYMENT SCHEDULE - VIEW </h4>
			     ...
			     <c:set var="active" scope="page" value="N" />
			     
			     <table id="schedule-list" class="table table-hover table-striped table-condensed responsive">  
				 <thead>
				      <tr>  
				       <th>SN</th>  
				       <th>Interest</th>  
				      ...
				      <c:forEach items="${schedules}" var="item">  
						...
						<td align="left">
						       <c:if test="${active=='Y'}">
							       <c:if test="${item.paymentStatus=='P'}"> 
								   PAID
							       </c:if>
						       ...
						       <c:if test="${active=='N'}"> 
							       ...
							       <c:if test="${item.paymentStatus!='P'}"> 
								    <a href="newLoanSchedulePayment.htm?id=${item.id}">PAY</a>
								    <c:set var="active" scope="page" value="Y" />
							       </c:if>
						       </c:if>
