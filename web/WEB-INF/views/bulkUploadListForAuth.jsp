		       ...
		       <c:if test="${!empty bulkuploads}">  
					...
					<tbody>
					      <c:forEach items="${bulkuploads}" var="item">  
							...
							<td><a href="approveBulkUpload.htm?id=${item.id}">${item.batchId}</a></td>  
							...
							<td>${uT} || 
								    ...
								    <c:if test="${item.verifiedBy == logonUser}">
								    ...
								    <c:if test="${item.verifiedBy ne logonUser}">
								       <a href="approveBulkUpload.htm?id=${item.id}">APPROVE</a></td> 
						    		    </c:if>							
