...
<div class="media-body">
    <div style="float:left">
		...
		<li><a href="#">Home</a></li>
		<li>Loan Request For Approval</li>
	    </ul>
	    <h4>Approve Loan Request(s)</h4>
    </div>
        ...
        <div class="col-md-12">
	       ...
	       <div class="panel-body">
		      ...
		      <script type="text/javascript" src="<%=request.getContextPath()%>/resources/js/jquery-ui-i18n.min.js"></script> 
			     ...
			     <div class="media-body">
				<h4>LIST OF PENDING LOAN REQUESTS</h4>
			     </div>
				...
				--%>
					...
					<td><fmt:formatNumber type="number" maxFractionDigits="2" value="${loan.requestedAmount}" /></td>  
					<td><c:out value="${loan.appliedRate}"/></td> 
					<td><c:out value="${loan.loanCaseId}"/></td>  
					<td><c:out value="${loan.loanStatusDesc}"/></td>  
					<td align="center">
					    <c:if test="${loan.requestBy == logonUser}">
					       <font color="red"><div data-toggle="tooltip" title="A different user is required to approve this Loan" cssClass="tooltips">NOT ALLOWED</div></font>
					    </c:if>
					    <c:if test="${loan.requestBy ne logonUser}">
					       <a href="approveLoanRequest0.htm?id=${loan.id}">APPROVE</a>
					    </c:if>
					</td>  
				       </tr>  
				      </c:forEach>  
		       ...
		       <c:if test="${empty loanRequests}">  
				...
				<h4>THERE IS NO PENDING LOAN REQUEST TO APPROVE</h4>
			     </div>
		       </c:if>
               </div>
            </div><!-- col-md-6 -->
