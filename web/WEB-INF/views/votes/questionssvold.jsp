...
<div class="contentpanel">
    <div class="row">
                           <div class="col-md-8">
                                ...
                                <!-- BASIC WIZARD -->
                                <form:form method="post" action="saveVotessv.htm" id="basicWizard" class="panel-wizard" modelAttribute="membervote" >
    							...
    							<div class="form-group">
	                                                ...
	                                                <div class="col-sm-8">
	                                                     ...
	                                                     <c:choose>
                                                                 <c:when test="${dvotedetails.electionanswertypeid == 2}">
                                                                     <input name="${query2}" type="checkbox" value="${optionitem.id}" class=" col-sm-4" />&nbsp;&nbsp;&nbsp;&nbsp;
                                                                     <input name="txt:::${dvotedetails.id}:::${optionitem.id}" type="text"  min="0"  value="0" size="50" class="col-sm-4 "  />
                                                                 </c:when>
                                                                 <c:otherwise>
                                                                     <input name="${query2}" type="radio" value="${optionitem.id}" class="col-sm-4 " />&nbsp;&nbsp;&nbsp;&nbsp;
                                                                     <input name="txt:::${dvotedetails.id}:::${optionitem.id}" type="text" min="0" value="0" size="50" class="col-sm-4"  />
                                                                 </c:otherwise>
                                                             </c:choose>                           
                                    ...
                                    <ul class="list-unstyled wizard">
                                         ...
                                         <input type="hidden" name="usrnoofvotes"  id="usrnoofvotes" value="${agmvotesession.noofvoteunits}" />
 ...
 <script>
            jQuery(document).ready(function() {
                ...
                jQuery('#basicWizard').bootstrapWizard({
                    ...
                    onTabClick: function(tab, navigation, index) {
                      ...
                      var $objsub = $($dtab).find($obj);
                       ...
                       var $arr = $objsub.map(function () {
                        ...
                        for(var $i=0;$i<$arr.length;$i++){
                         if(parseInt($arr[$i]))
                           {  
                            $totalx = $totalx + parseInt($arr[$i]);
                           }
                           else {
                           if(parseInt($arr[$i]) < 0 || isNaN(parseInt($arr[$i]))) 
                               {
                                   alert("Invalid Vote units input - must be a number and >= 0") ;   
                                   return false;
                                //return 0;   
                               }
                          } 
                     ...
                     //console.log("total " + $totalx);
                          if($totalx > $('#usrnoofvotes').val())
                           {  
                            alert("Total vote unit specified is more than your vote units - " + $totalx + " > " + $('#usrnoofvotes').val());   
                ...
                jQuery('.panel-wizard').submit(function() {    
                    var retVal = confirm('This will submit the votes. Click OK to continue');
                    if( retVal == true )
                    {
                        return true;
                    }
