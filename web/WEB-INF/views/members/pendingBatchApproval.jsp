<%-- 
    Document   : pendingBatchApproval
    Created on : Dec 4, 2015, 5:35:59 AM
    Author     : baydel200
--%>
        ...
        <table  id="data-list" class="table table-striped table-bordered responsive" >       
            ...
            <tbody>
                <c:forEach var="file" items="${listFiles}" >
                    <tr>
                        <td wrap>${file.originalFileName}</td>    
                        <td>${file.batchUploadReference.batchUploadReferenceId} </td>
                        <td>${file.batchUploadType.uploadTypeShort} </td>
                        <td>${file.batchRecordCount} </td>
                        <td>${file.createdDate}</td>
                        <td>${file.createdBy} </td>
                        <td><a href="${file.batchUploadType.viewValLink}?id=${file.batchUploadFileId}">View</a></td>
                        <td><a href="${file.batchUploadType.apprWithLink}?id=${file.batchUploadFileId}" onclick="" > Post</a></td>
                        <td><a href="${file.batchUploadType.apprWithoutLink}?id=${file.batchUploadFileId}" onclick="" >Approve</a></td>
                        <td><a href="${file.batchUploadType.apprRejectLink}?id=${file.batchUploadFileId}">Reject</a></td>
                    </tr>
                </c:forEach>
