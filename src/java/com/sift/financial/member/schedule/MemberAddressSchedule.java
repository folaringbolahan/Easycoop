 ...
 * @author baydel200
 */
public class MemberAddressSchedule extends BatchScheduleInterface {
    ...
    @Autowired
    private BatchAddressValidator batchAddressValidator;
   ...
   private MailNotificationImpl emailDispatcher;
    ...
    public List buildObjects(Object fileInfo, String path) {
        ...
        if (fileInfo instanceof BatchUploadFile) {
            ...
            fileUtil.setMemAddrType(memAddrType);
            try {
    ...
    public List processObjects(List dataList, Object fileInfo) {
        ...
        List finalObjects = new ArrayList<BatchAddressEntries>();
        ...
        for(Object record: dataList )
        {
            ...
            Date today = new Date();
            batchAddressValidator.setBranchId(fileInfoObj.getBranchId());
            batchAddressValidator.setCompanyId(fileInfoObj.getCompanyId());
            com.sift.financial.member.Company comp = getCompanyDAO().findById(fileInfoObj.getCompanyId());
            batchAddressValidator.setCountryId(comp.getCountries().getId().intValue());
            
            // generate address fieldList for country
            List<Map<String,Object>> addresList = getGenericConfigDAO().getAddressColumns(comp.getCountries().getId().intValue()+"");
            
            List<BatchAddressEntries> curBatches =  batchAddressValidator.doValidation(recordArray,addresList);
          
         for(BatchAddressEntries curBatch : curBatches)
         {
              ...
              // getBatchUploadFileDAO().save(fileInfoObj, sess);
               ...
               for(Object record : dataList)
               {
                 log.info("attending to record at position " + i);
                  ...
                  BatchAddressEntries batchMem = (BatchAddressEntries)record;
                 if(batchMem==null)
                 {
                 ...
                 getBatchAddressEntriesDAO().save(batchMem, sess);
    ...
    public BatchAddressValidator getBatchAddressValidator() {
        return batchAddressValidator;
    }

    public void setBatchAddressValidator(BatchAddressValidator batchAddressValidator) {
        this.batchAddressValidator = batchAddressValidator;
    }
