...
public class NotificationServicezk {
    @WireVariable("freemarkerConfiguration")
    Configuration freemarkerConfiguration;
    private TaskExecutor taskExecutor;
    String msgstr = "";
    String authenabled ="false";
    MailglBean MB;
    public NotificationServicezk() {
        MB = new MailglBean();
       MB = getMailConfig();
    }
  ...
  //   }
    ...
    public void preparemailandsend(String emailaddresses,String mailsubject,String mailtemplate,Map model){
        ...
        try {
           ...
           // System.out.println("freemarkerConfiguration " + freemarkerConfiguration.toString());
            try {
                Configuration frconfg = (Configuration) SpringUtil.getBean("freemarkerConfiguration");
                mailBody = FreeMarkerTemplateUtils.processTemplateIntoString(frconfg.getTemplate(template, "UTF-8"), model);
	...
	///System.out.println("Set message 2 ....");
		         ...
		         ///System.out.println("Set message 3....");
		         //Send the complete message parts
		         message.setContent(multipart );          
		         //message.setContent(mailBody, "text/html");

		         // Send message
                         //System.out.println("Set message 4....");
    ...
    public class sendMailTask implements Runnable {
        private MailglBean tMB;
        public sendMailTask(MailglBean tMB) {
            this.tMB = tMB;
        }
    ...
    public void sendMailasync(MailglBean ttMB) {
        //Thread trd1 = new Thread(new sendMailTask(ttMB));
        //System.out.println("Starting message thread ....");
        //trd1.start();
        TaskExecutor dtaskExecutor = (TaskExecutor) SpringUtil.getBean("taskExecutor");
        dtaskExecutor.execute(new sendMailTask(ttMB));
    }
