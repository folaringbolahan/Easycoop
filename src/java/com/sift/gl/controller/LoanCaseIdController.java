...
@Controller
//@RequestMapping("gl/")
public class LoanCaseIdController{
   ...
   @RequestMapping(value = {"loancaseid"}, method = {RequestMethod.GET,RequestMethod.POST})
   ...
   @RequestMapping(value = {"loandaterange"}, method = {RequestMethod.GET, RequestMethod.POST})
    public String loanlist(@ModelAttribute("reportpath") final Object mapping1FormObject, @ModelAttribute("paratype") final Object mapping2FormObject, ModelMap model) {
        Integer dcompany = user.getCurruser().getCompanyid();
        Integer dbranch = user.getCurruser().getBranch();
        reportdet = new AllItempara();
        ...
        model.addAttribute("reportpath", mapping1FormObject);
        model.addAttribute("paratype", mapping2FormObject);
        return "loandaterange";
    }
 
  

   @RequestMapping(value = {"loanmatcriteria"}, method = {RequestMethod.GET, RequestMethod.POST})
    public String loanmat(@ModelAttribute("reportpath") final Object mapping1FormObject, @ModelAttribute("paratype") final Object mapping2FormObject, ModelMap model) {
        Integer dcompany = user.getCurruser().getCompanyid();
        Integer dbranch = user.getCurruser().getBranch();
        reportdet = new AllItempara();
        List<Product> products = productService.listProductsByBranch(dbranch.toString());
