...
public class ApplyEntrysPA {
    ...
    public ApplyEntrysPA(java.util.List<Entry> vallEntrys, Txnsheader vdtxnHeader,Integer gYearx ,Integer gPeriodx,Integer branch,Integer companyid,String timezone) {
       try {
           dbobj.inimkcon();
    ...
    public boolean performAllupdates( ){
        ...
        for (Entry comEntry : allEntrys) {
          //  System.out.println("here now 1" + thisEntryvalid);
            ...
            if ((thisEntryvalid == false) || (thisEntryvalid1 == false)) {
           allEntrysvalid = false ;
    ...
    public boolean updateEntryHeader(Txnsheader dtxnHeader){
        ...
        else
        {
         ...
         try {
            mySQLString2 = "insert into txnsheaderua (txnSerial,txnType,txnDate,postdate,entrydate,docRef,headernarrative,source,userId,branchid,year,period,companyid) values('" + dtxnHeader.getTxnSerial() + "','" + dtxnHeader.getTxnType() + "',{d '" + tempDate1 + "'},{d '" + tempDate + "'},{d '" + tempDate2 + "'},'" + dtxnHeader.getDocRef() + "','" + dtxnHeader.getHeadernarrative() + "','" + dtxnHeader.getSource() + "','" + dtxnHeader.getUserId() + "','" + dtxnHeader.getBranchId() + "'," + dtxnHeader.getYear() + "," + dtxnHeader.getPeriod() + "," + vCompany + ")";
