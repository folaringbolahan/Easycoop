...
import com.sift.gl.ApplyEntrysPA;
...
@Path("/glwsua")
//@Autowire
public class TxnsdetRESTFacadePA {
    String msgstr = "";
    String refno = "";
    public TxnsdetRESTFacadePA() {
    }
    ...
    public Response create(Entrys entity) {
        ...
        if (retval == true) {
         // msg = Response.created(URI.create(msgstrx)).build(); //"OK";  
            msg = Response.created(URI.create(msgstrx)).entity(refno).build();
    ...
    public boolean updateEntrys(Entrys dEntrys){
        ...
        if ((dgenentrys.validateSumofAmount()== true)&&(dgenentrys.allAccountsvalid()== true)&&(dgenentrys.datesvalid(dEntrys.getTxnsheader())==true)){
              ...
              ApplyEntrysPA dataUpdate =  new ApplyEntrysPA(dgenentrys.getEntrys(),thetxnheader,thetxnheader.getYear(),thetxnheader.getPeriod(),thetxnheader.getBranchId(),thetxnheader.getCompanyId(),thetxnheader.getTimezone());
              if (dataUpdate.performAllupdates()== true) {
               msgstr = "Transaction with Ref.no " + dataUpdate.varSerial + " Saved";
               refno = dataUpdate.varSerial;
